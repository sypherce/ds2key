...
WPARAM GetMessages()
{
	...
	WPARAM message = PeekMessage(&messages, NULL_VALUE, 0, 0, PM_REMOVE);
	if(message == TRUE)
	{
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
		...
		case WM_CONTEXTMENU:
		{
			...
			if(hMenu)
			{
				...
				if(hmenuPopup)
				{
					...
					TrackPopupMenu(hmenuPopup, TPM_BOTTOMALIGN, pt.x, pt.y, 0, hwnd, NULL_VALUE);
				}
			}
			break;
		}
	...
	case WM_NOTIFY:
	{
		...
		if(events[event].object == nullptr)
			break;
