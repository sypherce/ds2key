...
namespace D2K {
	namespace Config {
		...
		int load() {
			...
			set_port(iniParser::getint(ini, "settings:port", DEFAULT_PORT));
			iniParser::freedict(ini);

			return 0;
		}

		int load_profile(ProfileData* profile_data, uint8_t profile_number) {
			...
			if(ini == NULL) {
				...
				save_profile(profile_data, profile_number);
		...
		int save_profile(ProfileData* Profile, uint8_t profileNumber) {
			std::ostringstream ssfilename;
